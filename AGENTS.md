...
**Version courante :** v4.14.0 (versionCode 28) — Room v30 (branche `florent/prive`).
...
| v26 → v27 | Alias de notification et clé de rapprochement Trade Republic sur les transactions |
| v27 → v28 | Corbeille des transactions |
| v28 → v29 | Catalogue de catégories et `categoryConfirmed` |
| v29 → v30 | `etf_plans` et `etf_purchases`, liés à un actif libre ; index unique de transaction validée |
...
- **Bump de version :** à chaque saut de fonctionnalité ou d'amélioration
  ...
  `AGENTS.md` annonçait même une version qui n'existait pas dans le code.
...
- Le mode ETF est défini par une ligne `etf_plans` liée à `custom_assets`, sans modifier l’ancien actif ni ses mouvements mensuels. Le capital de référence comprend tous les achats jusqu’à la date de référence incluse ; seuls les achats validés après cette date s’ajoutent au capital.
- `EtfRepositoryImpl` écrit les achats et les valorisations dans des transactions Room séparées. Il ne modifie jamais le solde bancaire ni les transactions importées lors de la validation d’un achat. Le calendrier est indicatif, sans écriture automatique.
- `etf_purchases.transactionId` est unique ; l’association d’un achat manuel à un import conserve montant, frais et date de l’achat. La réconciliation CSV/notification existante conserve l’identifiant de transaction. Une suppression bancaire ne supprime pas un achat validé ; retirer l’achat du suivi est une action explicite.
- Sauvegarde complète format 4 : inclure les deux tables ETF, valider références/doublons/montants avant restauration, insérer les actifs puis les plans puis les achats. Les formats 1 à 3 restent lisibles.
