...
class UpdateCustomAssetUseCase @Inject constructor(
    private val repository: CustomInvestmentRepository
) {
    suspend operator fun invoke(asset: CustomAsset): Result<Unit> {
        ...
        return runCatching { repository.updateCustomAsset(asset) }
    }
}
