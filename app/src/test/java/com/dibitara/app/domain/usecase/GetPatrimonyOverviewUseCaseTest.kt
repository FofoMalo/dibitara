...
class GetPatrimonyOverviewUseCaseTest {
    ...
    @BeforeEach
    fun setUp() {
        useCase = GetPatrimonyOverviewUseCase(budgetRepo, savingsRepo, investmentRepo, debtRepo)
    }

    @Test
    fun `calcule le patrimoine net en agrégeant toutes les sources`() = runTest {
        val budget = Budget(month = 5, year = 2026, allocatedCents = 300000L, spentCents = 100000L, currency = Currency.EUR)
        val savings = listOf(
        ...
        // liquidités = budget restant = 300000 - 100000 = 200000
        assertEquals(200000L, overview.liquiditesCents)
        assertEquals(500000L, overview.epargneCents)
        // investissements = immo + scpi = 20000000 + 200000 = 20200000
        ...
        // patrimoine brut = liquidités + épargne + investissements (pas Airbnb)
